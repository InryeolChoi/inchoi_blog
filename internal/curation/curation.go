...
var Moves = []Move{
	...
	// 클라우드는 `가상화기술` 밑에 Docker와 나란히 있었는데, 그 열세 편은
	// 가상화가 아니라 **기계를 빌려 쓰는 절차**다(인스턴스 만들기, SSH 키,
	// 방화벽, code-server·RStudio 깔기). 개발 도구를 모아둔 tooling이 제자리다.
	// **부모가 노션에서 온 2단계 분류다** — regroup의 moveTarget 참고.
	{SourceName: "클라우드 탐구생활", ToSlug: "tooling", Why: "가상화가 아니라 기계를 빌려 쓰는 절차라 개발 도구 쪽이 맞다"},
}
...
var DropCategories = []DropCategory{
	...
	// 빈 마디 둘(2026-09-02). `알고리즘: 이론` 아래 `기초 (1)` 7편과 `(2)` 2편이
	// **전부 본문 0바이트**라 함께 뺐다. 남는 것이 없으므로 분류도 없앤다 —
	// 덤프가 만든 마디라 0으로 남으면 그대로 막다른 길이다.
	{
		SourceName: "알고리즘 : 기초 (1)",
		Why:        "그 아래 글 일곱이 전부 본문 0바이트라 함께 뺐다. 남는 글이 없다",
	},
	{
		SourceName: "알고리즘 : 기초 (2)",
		Why:        "그 아래 글 둘이 전부 본문 0바이트라 함께 뺐다. 남는 글이 없다",
	},
}
...
var DropPosts = []DropPost{
	...
	// `알고리즘: 이론`의 빈 글 열다섯(2026-09-02). 전부 본문이 0바이트다.
	// **`시간복잡도`만 draft가 아니라 unlisted였다** — 제목만 있는 빈 페이지가
	// 공개 화면에 그대로 나가고 있었다.
	{NotionPageID: "2fdef346-d6d7-484f-9eb5-d6247f66a602", Title: "벨만포드 알고리즘", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "5f959cfb-c78f-4603-ac9e-ae989629b7f2", Title: "플로이드-워셜 알고리즘", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "32f2c96f-69b7-4019-a19d-27b09fd838d2", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "54224490-08e4-4ab6-80c4-530e5e88bb63", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "c88d843c-bc1d-4a63-b718-fb46211aa6ed", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "ee7a17d9-6730-43bf-9bb2-e140db8650d6", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "f6d2c6b3-e60d-48cd-bf78-410e5ce60f8e", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "dab68c85-2fb5-4c1e-8b86-c691f5a9e2ed", Title: "레드-블랙 트리", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "2e237a49-aa42-45b0-aac5-1e6ad54f821a", Title: "이진트리", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "c16a4d8d-22b2-498d-bcf2-58db9c819cb6", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "c72a0966-aa49-4237-a641-6aaaec5429b4", Title: "(제목 없음)", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	{NotionPageID: "5e5820a8-fb42-4b3d-9926-5fdf086cff92", Title: "동적계획법", Why: "본문이 0바이트다. 제목만 있고 내용이 없다"},
	...
	// 수리통계1의 중복 아홉(2026-09-03). **노션 원본에 같은 글이 두 벌 있었다** —
	// 2022-08~09에 만든 `확률분포 정리` 절을 2022-12-30에 `6. 분포의 활용 : 예시`로
	// 통째로 복사한 것이라 본문 sha256까지 같다(CLAUDE.md "데이터 현황").
	//
	// **나중에 복사한 쪽이 아니라 원본 쪽을 뺀다.** `6. 분포의 활용 : 예시`는
	// 번호가 붙은 차례(1~6) 안에 있어 흐름이 자연스럽고, `확률분포 정리`는
	// 그 여덟 편만 나열하는 인덱스라 남길 값이 따로 없다. 인덱스 글도 함께
	// 뺀다 — 가리킬 글이 전부 없어지면 빈 목록만 남는다.
	{NotionPageID: "d2ccf8df-6689-45f0-b582-ab2d7297bf4c", Title: "F분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
	{NotionPageID: "543561ed-18b6-459d-a663-d0d0926d4529", Title: "t분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
	{NotionPageID: "22378dd5-c04a-4d40-a42b-55ef11030f80", Title: "다변량 정규분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
	{NotionPageID: "1b038bf0-1c47-4314-a358-50c8340927d7", Title: "다항분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
	{NotionPageID: "9dc43255-9c1e-4ce0-90f3-c094b75a028b", Title: "베타분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
	{NotionPageID: "cd4a6e90-cc4a-4680-9def-f0c698994270", Title: "이변량 정규분포", Why: "`6. 분포의 활용 : 예시` 밑에 본문이 글자까지 같은 글이 있다"},
...
var baseBodyEdits = []BodyEdit{
	...
	// `알고리즘: 이론` 표지. 하위 마디 여섯으로 가는 목록인데, `기초 (1)`과
	// `기초 (2)`는 그 아래 글 아홉이 **전부 본문 0바이트**라 통째로 없앴다.
