...
# 글감함의 "초안 생성"이 쓰는 OpenRouter 키는 **여기 적지 않는다.**
# GitHub Actions의 secret(OPENROUTER_API_KEY)에서 배포가 /etc/blog/openrouter.env로
# 넣는다(.github/workflows/deploy.yml). 이 파일은 사람이 손으로 적는 자리다.
#
# 쓸 모델과 프롬프트는 키와 달리 secret이 아니라서 admin 환경설정 화면에서 고친다
# (settings 표의 ai.model / ai.prompt). 키만 서버에 있으면 된다.
