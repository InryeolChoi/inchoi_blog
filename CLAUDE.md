...
- **노션 최상위 페이지 19개는 자기 카테고리의 "표지 글"로 붙는다.** 경로가 자기
  제목뿐이라 조상이 없지만, 그 제목이 하위 글들의 경로에서 카테고리 이름으로
  등장한다. 그 이름과 맞으면 같은 이름의 카테고리에 붙인다.
  **스키마에는 표지 글임을 표시할 자리가 아직 없다** — 나중에
  `categories.cover_post_id`나 `posts.is_cover` 같은 걸 더할 일이다.
  지금은 `category_id`만 이어져 있고, 구분하려면 `original_path = title`로 찾으면 된다.
- `category_id`가 NULL인 글은 현재 0건이다.
